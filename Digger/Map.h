...
public:
	...
	void drawMap();
	void digTile(int x, int y, int direction, int& points);

	void updateGraph(int xpos, int ypos); // based: 
	
	//ENEMYYYYYYYYYYYYYYYYYYY
	bool isWalkable(int x, int y);
	vector<SDL_Point> findPath(SDL_Point start, SDL_Point goal);
	SDL_Point getNextStep(SDL_Point enemy, SDL_Point player);
